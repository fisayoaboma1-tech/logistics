...
const FAQItem = ({ question, answer, isOpen, onToggle }: FAQItemProps) => {
  ...
  return (
    <div
      className={`w-full glass shadow-[0px_2px_4px_rgba(0,0,0,0.16)] overflow-hidden rounded-[10px] transition-all duration-500 ease-out cursor-pointer card-hover ${
        isOpen ? "border-primary/30" : ""
      }`}
      onClick={handleClick}
    >
      <div className="w-full px-5 py-[18px] pr-4 flex justify-between items-center gap-5 text-left transition-all duration-300 ease-out">
        <div className="flex-1 text-foreground text-base font-medium leading-6 break-words tracking-tight">
          {question}
        </div>
    ...
    <section id="faq-section" className="w-full pt-[66px] pb-20 md:pb-40 px-5 relative flex flex-col justify-center items-center">
      ...
      <div className="self-stretch pt-8 pb-8 md:pt-14 md:pb-14 flex flex-col justify-center items-center gap-2 relative z-10">
        <div className="flex flex-col justify-start items-center gap-4">
          <h2 className="w-full max-w-[435px] text-center text-foreground text-4xl font-semibold leading-10 break-words heading-tight">
